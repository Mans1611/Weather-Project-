...
const keyAPI = ',us&appid=f6a02d657edf38d2610187e9f0907c82&units=metric';
...
button.addEventListener('click',()=>{
    ...
    if(zipCode == ''){
        ...
        setTimeout(()=>message.innerHTML  = 'You have not entered a Zip code',100);
    }
        ...
        .then(function(allData){
            if (allData.cod == 404){   // to check if the zip code is exist or not 
                message.classList.remove('display-none');
                message.classList.add('message');
                setTimeout(()=>message.innerHTML  = 'The Zip code is not found',100);
                entry.classList.add('display-none');
            }
                ...
                postData({
                temprature : Math.round(allData.main.temp),
                city: allData.name,
                fellings : val,
...
const getSpesificInfo = async ()=>{
    ...
    try{
        ...
        document.getElementById('content').innerHTML  = `Your fellings : ${allData.fellings} `;
        document.getElementById('city').innerHTML  = `The City name is : ${allData.cityName}`;
        document.getElementById('date').innerHTML  = `The Date is : ${newDate}`;
        document.getElementById('temp').innerHTML  =  `The Temprature now is : ${Math.round((allData.tempreture))} C`; //to convert to to celisus 
        document.getElementById('sky').innerHTML  = `The Sky today is ${allData.sky} `;
